...
# Base directory of *your* project (where this pt_hub.py lives)
THIS_DIR = os.path.dirname(os.path.abspath(__file__))

DEFAULT_SETTINGS = {
    # Use the local project folder by default (no developer machine paths)
    "main_neural_dir": THIS_DIR,
    "coins": ["WLFI"],
...
class PowerTraderHub(tk.Tk):
    ...
    def open_settings_dialog(self) -> None:
        ...
        def _open_api_folder() -> None:
            """Open the folder where bybit_key.txt / bybit_secret.txt live."""
            try:
        ...
        def _clear_api_files() -> None:
            """Delete bybit_key.txt / bybit_secret.txt (with a big confirmation)."""
            key_path, secret_path = _api_paths()
            ...
            _refresh_api_status()
            messagebox.showinfo("Deleted", "Deleted bybit_key.txt and bybit_secret.txt.")

        def _open_robinhood_api_wizard() -> None:
            """
            Bybit API setup wizard (replaces old Robinhood wizard).

            Saves:
              - bybit_key.txt    (API Key)
              - bybit_secret.txt (API Secret)
            """
            ...
            frm = ttk.Frame(wiz, padding=12)
            frm.pack(fill="both", expand=True)

            ttk.Label(
                frm,
                text="Paste your Bybit API Key + Secret below. They will be saved as bybit_key.txt and bybit_secret.txt in this project folder.\n\n"
                     "Tip: Create the key in Bybit with Spot trading permissions and consider IP restrictions.",
                justify="left"
            ).pack(anchor="w", pady=(0, 12))

            existing_k, existing_s = _read_api_files()
            key_var = tk.StringVar(value=existing_k)
            sec_var = tk.StringVar(value=existing_s)

            row = ttk.Frame(frm)
            row.pack(fill="x", pady=6)
            ttk.Label(row, text="Bybit API Key:", width=16).pack(side="left")
            ttk.Entry(row, textvariable=key_var).pack(side="left", fill="x", expand=True)

            row2 = ttk.Frame(frm)
            row2.pack(fill="x", pady=6)
            ttk.Label(row2, text="Bybit API Secret:", width=16).pack(side="left")
            ttk.Entry(row2, textvariable=sec_var, show="•").pack(side="left", fill="x", expand=True)
            ...
            def do_save():
                ...
                s = (sec_var.get() or "").strip()
                if not k or not s:
                    messagebox.showerror("Missing", "Please paste BOTH the API Key and API Secret.")
                    return
                _write_api_files(k, s)
                _refresh_api_status()
                messagebox.showinfo("Saved", "Saved bybit_key.txt and bybit_secret.txt.")
                wiz.destroy()

            ttk.Button(btns, text="Save", command=do_save).pack(side="left")
            ttk.Button(btns, text="Close", command=wiz.destroy).pack(side="left", padx=8)
