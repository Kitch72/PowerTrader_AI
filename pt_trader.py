...
_BYBIT_KEY_PATH = os.path.join(_THIS_DIR, "bybit_key.txt")
_BYBIT_SECRET_PATH = os.path.join(_THIS_DIR, "bybit_secret.txt")

try:
    with open(_BYBIT_KEY_PATH, "r", encoding="utf-8") as f:
        BYBIT_API_KEY = (f.read() or "").strip()
    with open(_BYBIT_SECRET_PATH, "r", encoding="utf-8") as f:
        BYBIT_API_SECRET = (f.read() or "").strip()
except Exception:
    BYBIT_API_KEY = ""
    BYBIT_API_SECRET = ""
...
if not BYBIT_API_KEY or not BYBIT_API_SECRET:
    print(
        "\n[PowerTrader] Bybit API credentials not found.\n"
        f"Expected files:\n  {_BYBIT_KEY_PATH}\n  {_BYBIT_SECRET_PATH}\n\n"
        "Open the GUI and go to Settings → Bybit API → Setup / Update.\n"
        "That wizard will save bybit_key.txt + bybit_secret.txt so this trader can authenticate.\n"
    )
    raise SystemExit(1)

# --- Safety switch: real trading only if explicitly enabled ---
# Use: BYBIT_ENABLE_TRADING=1 python pt_hub.py
BYBIT_ENABLE_TRADING = (os.getenv("BYBIT_ENABLE_TRADING", "0").strip() == "1")

# Practical guardrails for volatile coins (WLFI)
MAX_SPREAD_PCT = float(os.getenv("MAX_SPREAD_PCT", "0.35"))     # 0.35% default
MIN_ORDER_NOTIONAL = float(os.getenv("MIN_ORDER_NOTIONAL", "5")) # 5 USDT default


class CryptoAPITrading:
    def calculate_cost_basis(self) -> Dict[str, float]:
        """
        Rebuild average entry price per coin at startup.

        IMPORTANT:
        The rest of this bot reads cost_basis using the asset code (e.g. "WLFI"),
        so this returns:
          { "WLFI": 0.1234, "BTC": 98000.0, ... }
        """
        cost_basis: Dict[str, float] = {}

        try:
            holdings = self.get_holdings() or {}
            rows = holdings.get("results", []) or []
        except Exception:
            rows = []

        for h in rows:
            asset = (h.get("asset_code") or "").upper().strip()
            if not asset or asset in ("USDT", "USDC"):
                continue

            rh_symbol = f"{asset}-USD"

            try:
                orders_resp = self.get_orders(rh_symbol) or {}
            ...
            for o in orders:
                try:
                    ...
                    for ex in execs:
                        q = float(ex.get("quantity") or 0.0)
                        p = float(ex.get("effective_price") or 0.0)
                        if q > 0 and p > 0:
                            total_qty += q
                            total_cost += q * p
                except Exception:
            ...
            if total_qty > 0:
                cost_basis[asset] = total_cost / total_qty
    ...
    def __init__(self):
        ...
        # Bybit REST base URL (v5)
        self.base_url = "https://api.bybit.com"
        self.recv_window = "5000"
        self.bybit_category = "spot"
    ...
    # Bybit helpers
    # -----------------------------
    def _rh_symbol_to_bybit(self, symbol: str) -> str:
        """
        Convert the bot's internal symbol format to Bybit spot symbol.
        Examples:
          "WLFI-USD" -> "WLFIUSDT"
          "WLFI"     -> "WLFIUSDT"
        """
    ...
    @staticmethod
    def _bybit_ok(resp: object) -> bool:
        """Return True if a Bybit REST response indicates success (retCode == 0)."""
        try:
    ...
    def _bybit_sign(self, payload: str, timestamp_ms: str, recv_window: str) -> str:
        return hmac.new(
    ...
    def _bybit_request(self, method: str, endpoint: str, params: Optional[dict] = None, body: Optional[dict] = None) -> Any:
        """
        Bybit v5 signing:
        ...
        if method == "GET":
            ...
            payload = f"{ts}{self.api_key}{rw}{query}"
            sign = self._bybit_sign(payload, ts, rw)
            headers = {
            ...
            payload = f"{ts}{self.api_key}{rw}{body_json}"
            sign = self._bybit_sign(payload, ts, rw)
            headers = {
                "X-BAPI-API-KEY": self.api_key,
                "X-BAPI-SIGN": sign,
        ...
        try:
            ...
            return {"retCode": -1, "retMsg": f"Non-JSON response: HTTP {resp.status_code}", "raw": resp.text}

        return data

    def _bybit_public_tickers(self, bybit_symbol: str) -> Any:
        url = f"{self.base_url}/v5/market/tickers?category={self.bybit_category}&symbol={bybit_symbol}"
    ...
    # -----------------------------
    # Normalized “Robinhood-shaped” wrappers (minimal downstream edits)
    # -----------------------------
    def get_account(self) -> Any:
        """
        Returns a dict with a 'buying_power' key (like your Robinhood code expects).
        We treat available USDT as buying power for spot.
        """
        data = self._bybit_request(
            "GET",
        ...
        buying_power = 0.0
        try:
            coins = (data.get("result", {}) or {}).get("list", [])[0].get("coin", [])
            for c in coins:
                if (c.get("coin") or "").upper() == "USDT":
                    # prefer availableToWithdraw / availableBalance style fields; fall back safely
                    buying_power = float(
        ...
        # Robinhood-ish shape
        return {"buying_power": buying_power, "raw": data}

    def get_holdings(self) -> Any:
        """
        Returns dict {'results': [ {'asset_code': 'WLFI', 'total_quantity': ...}, ... ]}
        Filtered to the GUI-selected coins (WLFI only in your setup).
        """
        data = self._bybit_request(
            "GET",
            "/v5/account/wallet-balance",
        ...
        # Use GUI-selected coins as the allowlist (fallbacks apply if settings missing)
        try:
            allowed = set((_load_gui_settings() or {}).get("coins", []) or [])
            allowed = set([str(c).upper().strip() for c in allowed if str(c).strip()])
        except Exception:
            allowed = set()
        ...
        try:
            ...
            if lst:
                ...
                for c in coins:
                    asset = (c.get("coin") or "").upper().strip()
                    if not asset or asset in ("USDT", "USDC"):
                        continue
                    ...
                    if qty > 0.0:
                        results.append({"asset_code": asset, "total_quantity": qty})
        except Exception:
            results = []
    ...
    def get_trading_pairs(self) -> Any:
        """
        Your loop only needs this to be non-empty. We return the instruments list (spot).
        """
        data = self._bybit_request(
            "GET",
            "/v5/market/instruments-info",
            params={"category": self.bybit_category},
        )
        pairs = []
        try:
            pairs = ((data.get("result", {}) or {}).get("list", []) or [])
        except Exception:
            pairs = []
        return pairs
    ...
    def get_orders(self, symbol: str) -> Any:
        """
        Returns Robinhood-ish shape:
          {'results': [ {'side': 'buy'|'sell', 'state': 'filled'|..., 'created_at': iso, 'executions':[{'quantity':..,'effective_price':..}]} ]}
        """
        bybit_symbol = self._rh_symbol_to_bybit(symbol)
        data = self._bybit_request(
            "GET",
            "/v5/order/history",
            params={"category": self.bybit_category, "symbol": bybit_symbol, "limit": 50},
        )
        ...
        try:
            ...
            for it in items:
                ...
                state = "filled" if status == "filled" else status

                created_ms = int(it.get("createdTime") or 0)
                created_iso = datetime.datetime.utcfromtimestamp(created_ms / 1000.0).isoformat() + "Z" if created_ms else ""

                cum_qty = float(it.get("cumExecQty") or 0.0)
                avg_px = float(it.get("avgPrice") or 0.0)

                execs = []
                if cum_qty > 0.0 and avg_px > 0.0:
                    execs = [{"quantity": str(cum_qty), "effective_price": str(avg_px)}]
    ...
    def get_price(self, symbols: list) -> Dict[str, float]:
        """
        Replaces Robinhood best_bid_ask:
        returns (buy_prices, sell_prices, valid_symbols) where:
          buy_prices[s]  = ask
          sell_prices[s] = bid
        """
        buy_prices = {}
        ...
        for symbol in (symbols or []):
            try:
                bybit_symbol = self._rh_symbol_to_bybit(symbol)
                data = self._bybit_public_tickers(bybit_symbol)
                tick_list = (((data or {}).get("result", {}) or {}).get("list", []) or [])
                if not tick_list:
                    raise RuntimeError(f"No ticker for {bybit_symbol}")
                ...
                if ask > 0.0 and bid > 0.0:
                    ...
                    raise RuntimeError(f"Bad ask/bid for {bybit_symbol}: {t}")
    ...
    def place_buy_order(
        ...
        # Safety switch: do not place real orders unless explicitly enabled
        if not BYBIT_ENABLE_TRADING:
            return None

        # Determine base-asset qty from notional using current ask
        current_buy_prices, current_sell_prices, _ = self.get_price([symbol])
        ask = float(current_buy_prices.get(symbol, 0.0) or 0.0)
        bid = float(current_sell_prices.get(symbol, 0.0) or 0.0)
        if ask <= 0.0 or bid <= 0.0:
            return None

        # Spread guard (WLFI can have ugly spreads)
        mid = (ask + bid) / 2.0
        spread_pct = ((ask - bid) / mid) * 100.0 if mid > 0 else 999.0
        if spread_pct > MAX_SPREAD_PCT:
            return None

        # Min notional guard (Bybit API often rejects tiny orders)
        notional = float(amount_in_usd or 0.0)
        ...
        body = {
            ...
            "qty": str(round(qty, 8)),
            "timeInForce": "IOC",
            "orderLinkId": str(client_order_id or str(uuid.uuid4())),
        }

        resp = self._bybit_request("POST", "/v5/order/create", body=body)

        if not self._bybit_ok(resp):
            return None

        # Record for GUI history (estimated fill at ask)
        try:
            order_id = ((resp.get("result", {}) or {}).get("orderId")) if isinstance(resp, dict) else None
        except Exception:
            order_id = None
    ...
    def place_sell_order(
        ...
        qty = float(asset_quantity or 0.0)
        if qty <= 0.0:
            return None
        ...
        # Record for GUI history (estimated fill at expected_price if provided, else current bid)
        price_for_log = None
        if expected_price is not None:
            price_for_log = float(expected_price)
        else:
            _, sell_prices, _ = self.get_price([symbol])
            price_for_log = float(sell_prices.get(symbol, 0.0) or 0.0)
        ...
        self._record_trade(
            ...
            symbol=symbol,
            qty=float(round(qty, 8)),
            price=float(price_for_log) if price_for_log is not None else None,
            avg_cost_basis=float(avg_cost_basis) if avg_cost_basis is not None else None,
            pnl_pct=float(pnl_pct) if pnl_pct is not None else None,
            tag=tag,
            order_id=order_id,
    ...
    def make_api_request_legacy(self, method: str, path: str, body: Optional[str] = "") -> Any:
    ...
    def get_authorization_header_legacy(
    ...
    def get_account_legacy(self) -> Any:
        path = "/api/v1/crypto/trading/accounts/"
        return self.make_api_request("GET", path)

    def get_holdings_legacy(self) -> Any:
        path = "/api/v1/crypto/trading/holdings/"
        return self.make_api_request("GET", path)

    def get_trading_pairs_legacy(self) -> Any:
    ...
    def get_orders_legacy(self, symbol: str) -> Any:
    ...
    def get_price_legacy(self, symbols: list) -> Dict[str, float]:
        buy_prices = {}
        sell_prices = {}
        valid_symbols = []
            ...
            if avg_cost_basis > 0:
                ...
                if state["active"]:
                    ...
                    if state["was_above"] and (current_sell_price < state["line"]):
                        ...
                        response = sell_resp = self.place_sell_order(
                            str(uuid.uuid4()),
                            "sell",
                            "market",
                            full_symbol,
                        ...
                        if self._bybit_ok(sell_resp):
                            trades_made = True
                            self.trailing_pm.pop(symbol, None)  # clear per-coin trailing state on exit
                        else:
                            # If sell failed, keep trailing state so we can retry next tick
                            pass
