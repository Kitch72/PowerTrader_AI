...
# -----------------------------
# Bybit market-data (current ASK)
#   GET /v5/market/tickers?category=spot&symbol=BTCUSDT
# -----------------------------
BYBIT_BASE_URL = "https://api.bybit.com"

def _rh_symbol_to_bybit(symbol: str) -> str:
...
def bybit_current_ask(symbol: str) -> float:
    """
    Returns Bybit current ask for Robinhood-style symbols like 'BTC-USD' (mapped to BTCUSDT).
    Uses Bybit PUBLIC market endpoint (no API key needed).
    """
    bybit_symbol = _rh_symbol_to_bybit(symbol)
    url = f"{BYBIT_BASE_URL}/v5/market/tickers?category=spot&symbol={bybit_symbol}"
    resp = requests.get(url, timeout=10)
    if resp.status_code != 200:
        raise RuntimeError(f"Bybit HTTP {resp.status_code}: {resp.text}")

    data = resp.json()
    tick_list = (((data or {}).get("result", {}) or {}).get("list", []) or [])
    if not tick_list:
        raise RuntimeError(f"Bybit tickers returned no results for {bybit_symbol}: {data}")

    ask = float(tick_list[0].get("ask1Price") or 0.0)
    if ask <= 0.0:
        raise RuntimeError(f"Bybit ask invalid for {bybit_symbol}: {tick_list[0]}")
    return ask
...
def step_coin(sym: str):
	...
	if tf_choice_index >= len(tf_choices):
		...
		while True:
			try:
				current = bybit_current_ask(rh_symbol)
				break
			except Exception:
				time.sleep(1)
				continue
